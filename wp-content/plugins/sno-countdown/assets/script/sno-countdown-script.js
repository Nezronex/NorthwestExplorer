jQuery(document).ready(function ($) {
    ...
    function updateCountDown(countDownDate, countdownDiv, countDownSeconds) {
        ...
        if (distance < 0) {
            $(countdownDiv).hide()
            $(countdownDiv).siblings('.sno-countdown-complete').show()
            return
        }
        ...
        $(countdownDiv).empty()
        $(countdownDiv).append(sno_countdown_container)
    }

    $('.sno-countdown').each(function () {
        var countDownDate = new Date($(this).data('countdown-date')).getTime()
        var countDownDiv = $(this)
        var countDownSeconds = $(this).data('countdown-seconds');
    ...
    function check_for_three_digits(countDownDiv) {
        var tiles = $(countDownDiv).find("div.sno-countdown-component-header:contains('Days')").closest('.sno-countdown-component').find('.sno-countdown-component-numbers div').length
        console.log( tiles )
        if( tiles > 2 ) {
            $(countDownDiv).find("div.sno-countdown-component-header:contains('Days')").closest('.sno-countdown-component').find('.sno-countdown-component-numbers').css('grid-template-columns', 'repeat(' + tiles + ', 1fr)');
        }
    }
})
